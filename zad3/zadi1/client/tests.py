...
def expect_error(name, fn, expected_code):
    try:
        res = fn()
        got = getattr(res, "errorCode", None) if res is not None else None
...
def main():
    ...
    try:
        ...
        check(
            ...
            base.ice_id() == "::library::Catalog",
            f"ice_id()={base.ice_id()!r}",
        )
        check("ice_isA(library::Catalog) is True", base.ice_isA("::library::Catalog"))
        check("ice_isA(library::NoSuchType) is False", not base.ice_isA("::library::NoSuchType"))

        prx = library.CatalogPrx.checkedCast(base)
        check("checkedCast to CatalogPrx works", prx is not None)
        ...
        for t in titles:
            res = prx.addBook(library.AddBookRequest(t, author, 2026, ["tag1", "tag2"]))
        ...
        expect_error(
            ...
            lambda: prx.addBook(library.AddBookRequest(titles[0], author, 2026, [])),
            "ALREADY_EXISTS",
        )
        expect_error(
            "AddBook empty title -> INVALID_ARGUMENT",
            lambda: prx.addBook(library.AddBookRequest("", "x", 1, [])),
            "INVALID_ARGUMENT",
        )
        expect_error(
        ...
        expect_error(
            ...
            lambda: prx.addBook(library.AddBookRequest(f"x{suffix}", f"y{suffix}", -1, [])),
            "INVALID_ARGUMENT",
        )

        found = call_find_by_author(communicator, adapter, prx, author, 0)
        check(
        ...
        found_one = call_find_by_author(communicator, adapter, prx, author, 1)
        check("FindByAuthor with limit=1 returns 1", len(found_one) == 1)

        found_none = call_find_by_author(communicator, adapter, prx, f"ZZZ-no-such-author{suffix}", 0)
        check("FindByAuthor with no match returns empty", len(found_none) == 0)

        try:
            call_find_by_author(communicator, adapter, prx, "", 0)
        ...
        for i in ids_added:
            res = prx.removeBook(i)
            check(f"RemoveBook id={i} succeeded", res.ok and res.errorCode == "")

        expect_error(
            "RemoveBook unknown id -> NOT_FOUND",
            lambda: prx.removeBook(10_000_000),
            "NOT_FOUND",
        )

        found_after = call_find_by_author(communicator, adapter, prx, author, 0)
        check("FindByAuthor after cleanup returns 0", len(found_after) == 0)

        suffix2 = f"{suffix}-ext"
        extra_ids = []

        zb = prx.addBook(library.AddBookRequest(f"ZeroYear{suffix2}", f"ZeroAuthor{suffix2}", 0, []))
        check("AddBook year=0 boundary accepted", zb.errorCode == "" and zb.bookId > 0)
        extra_ids.append(zb.bookId)

        nt = prx.addBook(library.AddBookRequest(f"NoTags{suffix2}", f"NoTagsAuthor{suffix2}", 2026, []))
        extra_ids.append(nt.bookId)
        got_nt = call_find_by_author(communicator, adapter, prx, f"NoTagsAuthor{suffix2}", 0)
        check(
        ...
        pb = prx.addBook(library.AddBookRequest(long_title_pl, polish_author, 1834, ["polski", "epopeja"]))
        extra_ids.append(pb.bookId)
        got_pl = call_find_by_author(communicator, adapter, prx, polish_author, 0)
        check(
        ...
        lb = prx.addBook(library.AddBookRequest(long_title, f"LongAuthor{suffix2}", 2026, []))
        extra_ids.append(lb.bookId)
        got_long = call_find_by_author(communicator, adapter, prx, f"LongAuthor{suffix2}", 0)
        check("Long (5KB) title round-trip intact", len(got_long) == 1 and got_long[0].title == long_title)

        ci_author = f"MixedCase{suffix2}"
        cb = prx.addBook(library.AddBookRequest(f"CI{suffix2}", ci_author, 2026, []))
        extra_ids.append(cb.bookId)
        got_lower = call_find_by_author(communicator, adapter, prx, ci_author.lower(), 0)
        got_upper = call_find_by_author(communicator, adapter, prx, ci_author.upper(), 0)
        ...
        got_sub = call_find_by_author(communicator, adapter, prx, f"ixedCase{suffix2}", 0)
        ...
        N = 50
        for i in range(N):
            r = prx.addBook(library.AddBookRequest(f"Bulk-{i:03d}{suffix2}", bulk_author, 2000 + i, []))
            bulk_ids.append(r.bookId)
        extra_ids.extend(bulk_ids)
        got_bulk = call_find_by_author(communicator, adapter, prx, bulk_author, 0)
        check(f"Large stream returns all {N} books", len(got_bulk) == N)
        got_bulk_lim = call_find_by_author(communicator, adapter, prx, bulk_author, 10)
        check("Large stream limit=10 returns 10", len(got_bulk_lim) == 10)
        got_bulk_neg = call_find_by_author(communicator, adapter, prx, bulk_author, -5)
        check("Large stream limit=-5 returns all (no-limit semantics)", len(got_bulk_neg) == N)

        high_year = prx.addBook(library.AddBookRequest(f"Y3000{suffix2}", f"FutureAuthor{suffix2}", 3000, []))
        extra_ids.append(high_year.bookId)
        got_future = call_find_by_author(communicator, adapter, prx, f"FutureAuthor{suffix2}", 0)
        check("AddBook large year=3000 accepted and preserved", len(got_future) == 1 and got_future[0].year == 3000)

        stats_before = prx.summary()
        expected_authors = {
